...
public class ConfigManager implements Util {
    ...
    public String config = "Tethys/config/";
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("Tethys").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("Tethys/" + name + "/"))) {
            this.config = "Tethys/" + name + "/";
        } else {
            this.config = "Tethys/config/";
        }
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("Tethys").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("Tethys/" + name + "/"));
    }
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("Tethys/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("Tethys", ""));
                writer.close();
            } else {
