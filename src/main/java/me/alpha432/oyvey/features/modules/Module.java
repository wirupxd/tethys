...
import me.alpha432.oyvey.features.modules.client.Modulee;
...
public class Module
    ...
    public void enable() {
        ...
        this.onToggle();
        this.onEnable();
        if (Modulee.getInstance().notifyToggles.getValue().booleanValue()) {
            if (Modulee.getInstance().Sound.getValue().booleanValue()) {
                TextComponentString text = new TextComponentString(OyVey.commandManager.getClientMessage() + " " + ChatFormatting.BOLD + this.getDisplayName() + ChatFormatting.GREEN + " enabled");
                Module.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion(text, 1);
                Modulee.mc.player.playSound(SoundEvents.BLOCK_NOTE_BELL, 1.0f, 1.0f);
            } else {
                TextComponentString text = new TextComponentString(OyVey.commandManager.getClientMessage() + " " + ChatFormatting.BOLD + this.getDisplayName() + ChatFormatting.GREEN + " enabled");
                Module.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion(text, 1);
            }
    ...
    public void disable() {
        ...
        if (Modulee.getInstance().notifyToggles.getValue().booleanValue()) {
            if (Modulee.getInstance().Sound.getValue().booleanValue()) {
                TextComponentString text = new TextComponentString(OyVey.commandManager.getClientMessage() + " " + ChatFormatting.BOLD + this.getDisplayName() + ChatFormatting.RED + " disabled");
                Module.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion(text, 1);
                Modulee.mc.player.playSound(SoundEvents.BLOCK_NOTE_BELL, 1.0f, 1.0f);
            } else {
                TextComponentString text = new TextComponentString(OyVey.commandManager.getClientMessage() + " " + ChatFormatting.BOLD + this.getDisplayName() + ChatFormatting.RED + " disabled");
