...
public class OyVeyGui
    ...
    private void load() {
        int n = -100;
        OyVey.moduleManager.getCategories().stream().sorted(Comparator.comparingInt(category -> category.getName().length())).forEach(category -> {});
        for (final Module.Category category2 : OyVey.moduleManager.getCategories()) {
            this.components.add(new Component(category2.getName(), n += 102, 4, true){

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    OyVey.moduleManager.getModulesByCategory(category2).forEach(module -> {
                        if (!module.hidden) {
